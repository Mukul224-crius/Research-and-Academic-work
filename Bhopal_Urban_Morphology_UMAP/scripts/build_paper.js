...
const methodsChildren = [
  ...
  p("7) OBF percentage and its normalized form, read directly from the grid attribute fields obf_pct and obf_n:"),
  eq("OBF%_i = (A_OBF,i / A_grid,i) × 100,     OBF_n,i = OBF%_i / 100", "7"),
  p("verified against the attribute table (obf_pct ranges 0-53.151, obf_n correspondingly 0-0.532)."),
  p("8) WSF normalization: the WSF layer is a binary settlement mask (DN = 255 built-up, DN = 0 otherwise); the stored field wsf_n is consistent with the built-up pixel fraction per cell,"),
  eq("WSF_n,i = N_built-up,i / N_valid,i", "8"),
  flag("the precise QGIS zonal-statistics expression that generated wsf_n was not recovered from the delivered shapefile -- Eq. (8) is the conceptually consistent form, not a confirmed source expression"),
  p("9) Urban Morphological Index (UMI): reverse-derived and numerically verified against every one of the 423 valid grid cells in the delivered attribute table (e.g., cell id 247: obf_n=0.009, bld_n=0.009, wsf_n=0.988 → UMI=0.335 exactly as stored),"),
  eq("UMI_i = (OBF_n,i + bld_n,i + WSF_n,i) / 3", "9"),
  p("10) Weighted UMI (UMI_w): unlike UMI, this field is not a simple mean. We recovered its weighting by ordinary least-squares regression of UMI_w on (OBF_n, bld_n, WSF_n) with no intercept across all 423 valid cells, obtaining weights of 0.400, 0.400, and 0.200 respectively, with a maximum absolute residual of 0.00046 across every cell (consistent with field rounding rather than model error):"),
  eq("UMI_w,i = 0.4·OBF_n,i + 0.4·bld_n,i + 0.2·WSF_n,i", "10"),
  flag("Eq. (10) is an empirically recovered fit (n=423, max abs. residual 0.00046), not a value confirmed from the original processing script -- state this explicitly in the paper and, if possible, confirm against the source QGIS model before submission"),
  ...
  eq("min_{C_k} Σ_{k=1}^{K} Σ_{x_i ∈ C_k} ‖x_i − μ_k‖² ,  K=6", "13"),
  ...
  p("Two distinct Moran's I analyses are reported and are kept explicitly separate because they were not necessarily built with the same spatial-weights definition. First, a Local Moran's I (LISA) map was produced in the original GIS workflow (Fig. 3), classifying each cell into High-High, Low-High, Low-Low, or High-Low quadrants at p<0.05."),
  flag("the spatial-weights specification (queen/rook contiguity, k-nearest-neighbor, or distance band) used to generate the original LISA shapefile was not recoverable from the delivered files and must be confirmed by the authors from the original QGIS project before submission"),
  p("Second, as an independent validation check for this study, we separately computed a global Moran's I on the composite UMI surface using an explicitly defined rook (4-neighbor edge) contiguity matrix built directly from the regular grid's row and column indices, row-standardized, with significance assessed via a 999-permutation test rather than an assumed normal approximation. The general form of Moran's I is [12], and of the local statistic [3], [11]:"),
  eq("I = (n/W) · ΣᵢΣⱼ wᵢⱼ(xᵢ−x̄)(xⱼ−x̄) / Σᵢ(xᵢ−x̄)² ,  W=ΣᵢΣⱼwᵢⱼ", "14"),
  eq("Iᵢ = zᵢ Σⱼ wᵢⱼ zⱼ ,  zᵢ = (xᵢ − x̄)/s", "15"),
];
...
const resultsChildren2 = [
  ...
  h2("D. Limitations"),
  p("This analysis is cross-sectional and does not capture temporal morphological change. The 1 km grid risks an ecological-fallacy effect in which within-cell heterogeneity is averaged out. The k=6 solution was justified on interpretability rather than statistical optimality (Section IV-B). The WSF zonal-statistics expression (Eq. 8), the UMI_w weighting (Eq. 10, empirically recovered but not source-confirmed), the original LISA spatial-weights definition (Section III-F), the precise OBF data provenance (Section III-A), and UMAP hyperparameters (Section III-C) all require author confirmation before submission, as flagged in place above. Cluster labels have not yet been validated against independent ground-truth imagery or socioeconomic data."),
